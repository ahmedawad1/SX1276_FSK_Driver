...
uint8_t SX1272::ON()
{
  ...
  // 3.- SPI chip select // Powering the module
...
// from single_chan_pkt_fwd by Thomas Telkamp
  ...
  state = setFSK();
   ...
   writeRegister(0x1,0x11); // Modulation Type FSK, Gaussian Filter BT=0.5 , TX mode
   writeRegister(0x2,0x0D); // MSB BR
   writeRegister(0x3,0x05); // LSB BR
   writeRegister(0x4,0x01);  // MSB feq deviation
   writeRegister(0x5,0x99); // LSB freq deviation
   ...
   writeRegister(0x9,0x0); // PA output
   writeRegister(0xA,0x19); // RegPaRamp 
   writeRegister(0xB,0x1B); // RegOcp
   // =====Registers for the receiver ======
   writeRegister(0xC,0x20); // 
   writeRegister(0xD,0x0E); // 
   writeRegister(0xE,0x02); // 
   writeRegister(0xF,0x0A);
   writeRegister(0x10,0xFF);
   writeRegister(0x11,0x0);
   writeRegister(0x12,0x15);
   writeRegister(0x13,0x0B);
   writeRegister(0x14,0x28); // 
   writeRegister(0x15,0x0C);  // ooKFixedThreshold
   writeRegister(0x16,0x12);  // 
   ...
   writeRegister(0x1F,0xCA); // Preambledetect
   ...
   writeRegister(0x25,0x00); //MSB Preamble
   writeRegister(0x26,0x30); // LSB Preamble
   writeRegister(0x27,0xB4); //RegSyncConfig
   writeRegister(0x28,0x69); //RegSyncValue1
   writeRegister(0x29,0x81); //RegSyncValue2
   writeRegister(0x2A,0x7E); //RegSyncValue3
   writeRegister(0x2B,0x96); //RegSyncValue4
   writeRegister(0x2C,0x01);
   writeRegister(0x2D,0x01);
   writeRegister(0x2E,0x01);
   writeRegister(0x2F,0x01);
   writeRegister(0x30,0x50);   // RegPacketConfig1
   writeRegister(0x31,0x40);   // RegPacketConfig2
   writeRegister(0x32,0xF); // Payload length 
   writeRegister(0x33,0x00); // Node Address 
   writeRegister(0x34,0x00); // Broadcast address
   writeRegister(0x35,0x8F); // RegFifoThresh
   // ======sequencer registers ======
...
uint8_t SX1272::getPayloadLength()
{
	...
	//#if (SX1272_debug_mode > 1)
		...
		printf("0X%X = %d", _payloadlength,_payloadlength);
		printf(" ##\n");
		printf("\n");
	//#endif
...
{
	uint16_t length;
		printf("in 'setPacketLength()' _payloadlength = %d \n",_payloadlength);
...
uint8_t SX1272::setPayload(char *payload)
{
	...
	printf("in setPayload(char *payload) before truncPayload   _payloadlength; %d  and input payload length : %d\n",_payloadlength, length16);
	state = truncPayload(length16);
	printf("in setPayload(char *payload) after truncPayload   _payloadlength; %d \n",_payloadlength);
	if( state == 0 )
	{
...
uint8_t SX1272::setPacket(uint8_t dest, char *payload)
{
	...
	if( state == 0 )
	{
		...
		printf("before writting to the sender FIFO, the _payloadlength = %d bytes \n",_payloadlength); // Bigdutchman
		for(unsigned int i = 0; i < _payloadlength; i++)
		{
	 ...
	 // clearFlags();	// Initializing flags
	  ...
	  if( bitRead(value, 3) == 1 )
	  {
		  ...
		  //#if (SX1272_debug_mode > 1)
			  ...
			  printf("\n");
			  printf("Actual time for transmission: %d \n",millis() - current);
		 // #endif
	  }
	  else
	  {
